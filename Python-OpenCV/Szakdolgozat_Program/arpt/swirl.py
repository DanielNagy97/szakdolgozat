...
class Swirl(object):
    ...
    def calc_swirl(self, grid, eps=3):
        ...
        :return: None
        """
        # NOTE:It's only working for one swirl point only
        # and needs linear interpolation or some other smoothing methods!
        ...
        for k in range(len(grid.new_points)):
            ...
            if v.get_vector_lenght(current_vector) >= eps:
                direction_vectors = np.append(direction_vectors,
                                              np.array([current_vector]),
                                              axis=0)
                b = np.append(b, np.sum(np.multiply(current_vector,
                                                    grid.old_points[k])))

        intersections = np.empty((0, 2), dtype=np.int32)

        for i in range(len(b)-1):
            x = np.linalg.solve([direction_vectors[i], direction_vectors[i+1]],
                                [b[i], b[i+1]])
        ...
        if len(intersections) > 0:
            self._points = np.uint16(np.divide(intersections.sum(axis=0),
                                               len(intersections)))
